...
private_key:
	 openssl ecparam -out outputs/dev.key -name prime256v1 -genkey

csr:
	openssl req -new -key outputs/dev.key -out outputs/dev.csr

crt:
	openssl x509 -req -in outputs/dev.csr -CA outputs/ca.pem -CAkey outputs/ca.key -CAcreateserial -out outputs/dev.crt -days 90 -sha256
